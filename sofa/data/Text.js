...
//    icon            : "/rsl_shared/style/Axialis/Png/16x16/Globe.png",
    ...
    properties      : {
        ...
        session             : { label: "Some functions (e.g. allowed() and allowedURL()) require a session object, which is usually obtained from 'owner.page.session', where 'owner' is the fieldset to which this field belongs (see x.base.Base); if not part of a fieldset, 'session' can be provided directly", type: "x.Session", usage: "optional in spec" },
        ...
        fixed_key           : { label: "Indicates that this is a populated key field which corresponds to a database record, hence cannot be changed, set by x.data.Entity.populate()", type: "boolean", usage: "read only" },
...
x.data.Text.instantiate = function () {
    x.log.functionStart("instantiate", this, arguments);
};


x.data.Text.getControl = function () {
...
x.data.Text.getNumber = function (def_val) {
    var number_val;
...
x.data.Text.isBlank = function (val) {
...
x.data.Text.setInitial = function (new_val) {
...
x.data.Text.set = function (new_val) {
...
x.data.Text.setInternal = function (new_val) {
    var old_val;
...
//TODO
x.data.Text.setInternal.doc = {};


x.data.Text.beforeChange = function (new_val) {
    x.log.functionStart("beforeChange", this, arguments);
};
x.data.Text.beforeChange.doc = {
...
x.data.Text.beforeTransChange = function (new_val) {
    x.log.functionStart("beforeTransChange", this, arguments);
};
x.data.Text.beforeTransChange.doc = {
    purpose: "Activity to take place before field's value is changed - for overriding, nothing done here",
    args   : "Field's new value, to be set afterwards",
    returns: "nothing"
};
...
x.data.Text.afterChange = function (old_val) {
    x.log.functionStart("afterChange", this, arguments);
};
x.data.Text.afterChange.doc = {
    purpose: "Activity to take place before field's value is changed - for overriding, nothing done here",
...
x.data.Text.afterTransChange = function (old_val) {
    x.log.functionStart("afterTransChange", this, arguments);
};
x.data.Text.afterTransChange.doc = {
    purpose: "Activity to take place before field's value is changed - for overriding, nothing done here",
    args   : "Field's old value",
    returns: "nothing"
};
...
x.data.Text.isChangedSincePreviousUpdate = function () {
    x.log.functionStart("isChangedSincePreviousUpdate", this, arguments);
    return (this.prev_val !== this.get());
};
x.data.Text.isChangedSincePreviousUpdate.doc = {
...
x.data.Text.getId = function () {
    x.log.functionStart("getId", this, arguments);
    return this.id;
};
x.data.Text.getId.doc = {
...
x.data.Text.setProperty = function (name, val) {
...
x.data.Text.getKeyPieces = function () {
    x.log.functionStart("getKeyPieces", this, arguments);
    return 1;
};
x.data.Text.getKeyPieces.doc = {
...
x.data.Text.validate = function () {                    // validate() also responsible for setting text
...
x.data.Text.isModified = function () {
    x.log.functionStart("isModified", this, arguments);
    return this.modified;
};
x.data.Text.isModified.doc = {
...
x.data.Text.getTextFromVal = function () {
    var out = this.val;
...
x.data.Text.getTextFromVal.doc = {
...
x.data.Text.getURLFromVal = function () {
    x.log.functionStart("getURLFromVal", this, arguments);
    return "";
};        
x.data.Text.getURLFromVal.doc = {
...
x.data.Text.getConfigItemText = function (val) {
    var obj,
        label_prop = this.label_prop || "title";
    x.log.functionStart("getConfigItemText", this, arguments);
    obj = x.base.Base.getObject(this.config_item);
    if (typeof val !== "string") {
        val = this.get();
    }
    if (typeof obj[val] !== "object") {
        return "[unknown]";
//        throw new Error("unrecognised config item: " + this.config_item);
    }
    if (typeof obj[val][label_prop] !== "string") {
        return "[unknown]";
//        throw new Error("config item has no title: " + this.config_item);
    }
    return obj[val][label_prop];
};
x.data.Text.getConfigItemText.doc = {
...
x.data.Text.getUpdateText = function () {
    x.log.functionStart("getUpdateText", this, arguments);
    return this.val;
};
x.data.Text.getUpdateText.doc = {
...
x.data.Text.isVisible = function (field_group, hide_blank_uneditable) {
...
x.data.Text.setVisEdMand = function (visible_editable, mandatory) {
...
x.data.Text.getData = function (obj) {
    if (!this.isBlank()) {
...
x.data.Text.getTokenValue = function (token_parts) {
    var out;
...
x.data.Text.fieldEvent = function (event_id, new_val) {
