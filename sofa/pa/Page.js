...
x.Page = x.Base.clone({
    id                      : "Page",
    tabs                    : x.OrderedMap.clone({ id: "Page.tabs" }),
...
x.Page.doc = {
    ...
    properties : {
        ...
        entity              : { label: "Primary entity to which page relates", type: "x.Entity", usage: "optional in spec" },
        requires_key        : { label: "Whether or not page needs a page_key", type: "boolean", usage: "optional in spec" },
        tabs                : { label: "OrderedMap of page tabs", type: "x.OrderedMap", usage: "use methods only" },
        sections            : { label: "OrderedMap of page sections", type: "x.OrderedMap", usage: "use methods only" },
        links               : { label: "OrderedMap of page links", type: "x.OrderedMap", usage: "use methods only" },
        buttons             : { label: "OrderedMap of page buttons", type: "x.OrderedMap", usage: "use methods only" },
...
x.Page.updateTabs = function (params) {
...
x.Page.moveToTab = function (tab_ref, page_button) {
    var tabs = this.tabs,
        curr_tab_ix,
...
x.Page.moveToFirstErrorTab = function () {
    var move_to_tab = 999,
        i,
        section;
...
x.Page.updateSections = function (params) {
    var i;
...
x.Page.updateTrans = function (params) {
    var first_warnings,
        save;
...
x.Page.instantiateWorkflow = function (record, wf_tmpl, wf_inst_ref_field) {
    var wf_inst;
...
//        throw x.Exception.clone({ id: "first_node_page_mismatch", wf_tmpl: wf_tmpl, record: record, this_page_id: this.id,
...
x.Page.instantiateWorkflow.doc = {
...
/*
    if (!this.trans.isValid()) {            // All errors should be reported "locally", if appropriate for user
        x.log.debug(this, "x.Page.save() exiting - trans is initially not valid");
...
//    try {
        this.presave();
        if (!this.trans.isValid()) {            // All errors should be reported "locally", if appropriate for user
            x.log.debug(this, "x.Page.save() cancelling - trans is not valid after presave()");
        ...
        if (!this.trans.isValid()) {            // failures in performing_wf_nodes[i].complete() are irreversible
            x.log.debug(this, "x.Page.save() cancelling - trans is not valid after performing_wf_nodes[...].complete()");
            throw { type: 'E', text: "Not saved" };
        }
...
//                this.session.messages.add({ type: 'I', text: "Saved <a class='css_undo_link' href='" + x.pages.ac_tx_undo.getSimpleURL(this.trans.id) + "&page_button=undo'>undo</a>" });
            } else {
...
//---------------------------------------------------------------------------------------  render
x.Page.render = function (element, render_opts) {
    var page_elem;
    this.render_error = false;
    x.log.functionStart("render", this, arguments);
    if (!this.active) {
        throw x.Exception.clone({ id: "page_not_active", page: this });
    }
...
x.Page.renderSections = function (page_elem, render_opts, page_tab_id) {
    var sections_elem,
        div_elem,
        row_span = 0,
        i,
...
x.Page.renderChallengeToken = function (page_elem) {
...
x.Page.renderButtons = function (page_elem, render_opts) {
    var buttons_elem,
        i;
...
x.Page.getSimpleURL = function (override_key) {
    var page_key;
...
x.Page.getPageTitle = function () {
    x.log.functionStart("getPageTitle", this, arguments);
    return this.full_title;
};
x.Page.getPageTitle.doc = {
...
x.Page.addPerformingWorkflowNode = function (inst_id, node_id) {
    var wf_inst,
        inst_node,
        param;
...
x.Page.tabs.add = function (spec) {
    var tab;
    x.log.functionStart("add", this, arguments);
    if (!spec.label) {
        throw "Tab label must be specified in spec";
    }
    tab = x.Page.Tab.clone(spec);
    x.OrderedMap.add.call(this, tab);
    return tab;
};
x.Page.tabs.add.doc = {
...
x.Page.Link.getURL = function (override_key) {
    ...
    if (this.url) {
        ...
        if (this.page_to && x.pages[this.page_to]) {
            url = x.pages[this.page_to].skin;
        }
...
x.Page.Link.getLabel = function () {
    ...
    if (!this.label && this.page_to) {
        this.label = x.pages[this.page_to].short_title || x.pages[this.page_to].title;
    }
    return this.label;
};
...
x.Page.Link.render = function (element, render_opts) {
    ...
    if (this.visible) {
        ...
        if (this.page_to) {
            if (!x.pages[this.page_to]) {
                throw x.Exception.clone({ id: "invalid_target_page", page_to: this.page_to });
            }
...
x.Page.Link.renderNavOption = function (ul_elem, render_opts, this_val) {
    ...
    if (this.nav_options !== false) {
        ...
        if (this.open_in_modal || (this.page_to && x.pages[this.page_to].open_in_modal)) {
...
x.Page.links.add = function (spec) {
    var link;
    x.log.functionStart("add", this, arguments);
    if (!spec.page_to && !spec.label) {
        throw "Link page_to or label must be specified in spec";
    }
    link = x.Page.Link.clone(spec);
    x.OrderedMap.add.call(this, link);
    return link;
};
x.Page.links.add.doc = {
...
//---------------------------------------------------------------------------------------  Sections
x.Page.sections.add = function (spec) {
    var section;
    x.log.functionStart("add", this, arguments);
    if (!spec.type) {
        throw "Section type must be specified in spec: " + spec.id;
    }
    if (!x.sections[spec.type]) {
        throw "Section type not available: " + spec.type + " in spec: " + spec.id;
    }
    section = x.sections[spec.type].clone(spec);
    x.OrderedMap.add.call(this, section);
    return section;
};
x.Page.sections.add.doc = {
...
x.Page.buttons.add = function (spec) {
    var button;
    x.log.functionStart("add", this, arguments);
    if (!spec.label) {
        throw "Button label must be specified in spec: " + spec.id;
    }
    button = x.Page.Button.clone(spec);
    x.OrderedMap.add.call(this, button);
    return button;
};
x.Page.buttons.add.doc = {
...
x.ContextPage = x.Page.clone({
    id              : "ContextPage",
    requires_key    : true
});

//To show up in Chrome debugger...
//@ sourceURL=pa/Page.js