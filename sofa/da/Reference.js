...
//---------------------------------------------------------------------------- Reference
x.fields.Reference = x.fields.Text.clone({
    id              : "Reference",
    css_type        : "reference",
...
x.fields.Reference.doc = {
    location        : "x.fields",
    file            : "$Header: /rsl/rsl_app/core/fields/Reference.js,v 1.84 2014/07/22 07:50:55 francis Exp $",
    ...
    properties      : {
        ...
        lov                 : { label: "LoV object acting as a cache of the entity's records", type: "x.LoV", usage: "use methods only" },
...
x.fields.Reference.getKeyPieces = function () {
...
x.fields.Text.getDataLength = function () {
    ...
    if (typeof this.data_length !== "number") {
        if (!x.entities[this.ref_entity]) {
            throw x.Exception.clone({ id: "unrecognised_ref_entity", field: this, ref_entity: this.ref_entity });
        }
...
x.fields.Reference.getRefVal = function () {
    x.log.functionStart("getRefVal", this, arguments);
    return this.val;
};
x.fields.Reference.getRefVal.doc = {
...
x.fields.Reference.getLoV = function () {
    var condition;
    x.log.functionStart("getLoV", this, arguments);
    if (!this.lov) {
        if (!this.ref_entity) {
            throw x.Exception.clone({ id: "no_ref_entity_property", field: this });
        }
        if (!x.entities[this.ref_entity]) {
            throw x.Exception.clone({ id: "unrecognised_ref_entity", field: this, ref_entity: this.ref_entity });
        }
...
//      this.lov = x.LoV.getEntityLoV(this.ref_entity, this.getSession(), condition);
        if (condition) {
            this.lov = x.LoV.clone({ id: this.ref_entity, entity: this.ref_entity, condition: condition });
        } else {
            this.lov = x.LoV.getEntityLoV(this.ref_entity, this.getSession());
        }
    }
    return this.lov;
};
x.fields.Reference.getLoV.doc = {
...
x.fields.Reference.getOwnLoV = function (selection_filter) {
    x.log.functionStart("getOwnLoV", this, arguments);
    this.lov = x.LoV.clone({ id: this.ref_entity, entity: this.ref_entity });
    this.lov.loadEntity(null, selection_filter);
    return this.lov;
};
x.fields.Reference.getOwnLoV.doc = {
...
x.fields.Reference.getRow = function (require_trans) {
    var ref_val;
...
x.fields.Reference.validate = function () {
    var item,
        val;
...
/*
x.fields.Reference.getText = function () {
    var item;
    x.log.functionStart("getText", this, arguments);
    x.fields.Text.getText.call(this);
    if (!this.isBlank()) {
...
x.fields.Reference.linkToParent = function (parent_field, link_field, link_one_way) {
...
x.fields.Reference.linkToParent.doc = {
...
x.fields.Reference.afterChange = function (oldVal) {
...
// Called on the CHILD
x.fields.Reference.parentChanged = function () {
    var new_ref_condition,
        ref_row,
        implied_parent_val;
    x.log.functionStart("parentChanged", this, arguments);
    if (!this.link_field) {
        throw x.Exception.clone({ id: "invalid_configuration", text: "child field in linked pair is missing link_field" });
    }
...
x.fields.Reference.parentChanged.doc = {
...
x.fields.Reference.childChanged = function () {
    var ref_row,
        implied_parent_val;
    x.log.functionStart("childChanged", this, arguments);
    if (!this.linked_child) {
        throw x.Exception.clone({ id: "invalid_configuration", text: "parent field in linked pair is missing linked_child" });
    }
...
x.fields.Reference.childChanged.doc = {
...
x.fields.Reference.renderNavOptions = function (parent_elem, render_opts, primary_row) {
    var display_page,
        session,
        that = this,
        this_val,
...
x.fields.Reference.renderEditable = function (div, render_opts, inside_table) {
    x.log.functionStart("renderEditable", this, arguments);
    if (this.ref_entity && !x.entities[this.ref_entity]) {
        throw "Field " + this.toString() + " has unrecognised ref_entity: " + this.ref_entity;
    }
...
x.fields.Reference.renderAutocompleter = function (div, render_opts) {
    var input,
        input2;
...
x.fields.Reference.renderDropdown = function (div, render_opts) {
    var select;
...
x.fields.Reference.addClientSideProperties = function (span, render_opts) {
    ...
    x.fields.Text.addClientSideProperties.call(this, span, render_opts);
};

x.fields.Reference.autocompleter = function (match, out) {
    x.log.functionStart("autocompleter", this, arguments);
};

x.fields.Reference.addColumnToTable = function (query_table, col_spec) {
    var column,
        sort_cols;
    x.log.functionStart("addColumnToTable", this, arguments);
    if (!this.ref_entity || !x.entities[this.ref_entity]) {
        throw x.Exception.clone({ id: "invalid_ref_entity", ref_entity: this.ref_entity, field: this });
    }
...
x.fields.Reference.getReferentialIntegrityDDL = function () {
...
x.fields.Reference.checkDataIntegrity = function () {
    var resultset,
        out,
        count   = {},
        key_map = {},
...
x.fields.Reference.generateTestValue = function (session) {
    var lov,
        i;
    x.log.functionStart("generateTestValue", this, arguments);
    lov = x.LoV.getEntityLoV(this.ref_entity, session, this.generate_test_condition);
    if (!lov || lov.length() === 0) {
        return "";
    }
    i = Math.floor(Math.random() * lov.length());
    if (!lov.get(i)) {
        throw x.Exception.clone({ id: "invalid_lov_item", index: i, field: this });
    }
    return lov.get(i).id;
};

//To show up in Chrome debugger...
//@ sourceURL=da/Reference.js